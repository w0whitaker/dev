...
export default async function (eleventyConfig) {
	...
	eleventyConfig.addGlobalData("humanBuildDate", () => {
		let now = new Date();
		let sourceArray = new Intl.DateTimeFormat("en-us", {
			dayPeriod: "long",
			day: "2-digit",
			month: "long",
		}).formatToParts(now);
		...
		const conjunction = (period) => {
			if (period === "noon" || period === "midnight") {
				return ` at`;
			} else {
				return "";
			}
		...
		for (let i = 0; i < sourceArray.length; i++) {
			let { type, value } = sourceArray[i];
			if (type === "month") formattedDate.push(value);
			if (type === "day") formattedDate.push(` ${value}${ordinal(value)},`);
			if (type === "dayPeriod")
				formattedDate.push(`${conjunction(value)} ${value}`);
		}
